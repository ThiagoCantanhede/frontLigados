...
export default function GraficoEntrevistasAgendadas(props) {
  ...
  const montarGrafico = async () => {
    data.push(['Mês', 'Entrevistas agendadas', { role: 'style' }]);
    ...
    dadosAuditoria.data.map(async (da) => {
      if (tipo.agendamentoEntrevista === da.tipoAcao && da.anoAcao === ano) {
